...
class ImageHubController < ApiApplicationController
  ...
  def create
    ...
    Rails.logger.error "Error while creating a new image, error: #{e}, trace: #{e.backtrace}"
  ...
  def show
    ...
    Rails.logger.error "Error during show action, error: #{e}, trace: #{e.backtrace}"
    json_response({ error: ErrorMessages[:something_wrong] }, :internal_server_error)
  end
  ...
  def get_image_id
    json_response({ error: 'Image ID is a must' }, :not_acceptable) && return if params[:id].nil?
    @image_id = params[:id]
  end
end
